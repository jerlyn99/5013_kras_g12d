...
def main():
    ...
    chunk_size = 10000
    #chunk_files = []
    output_file_full = f"/hpctmp/{user}/predictions/predictions_final.csv"
    output_file_min = f"/hpctmp/{user}/predictions/predictions_minimal.csv"
    minimal_cols = [
    ...
    with ProcessPoolExecutor(max_workers=n_workers, 
        ...
        for i in range(0, len(lib), chunk_size):
            ...
            results = list(executor.map(process_molecule, tasks))

            print(f"Completed Batch {i//chunk_size}")
    ...
    print(f"High-confidence predictions: {high_confidence_count} compounds")
    #print()
...
if __name__ == "__main__":
    user = os.environ.get('USER')
    os.makedirs(f"/hpctmp/{user}/predictions", exist_ok=True)
    main()
